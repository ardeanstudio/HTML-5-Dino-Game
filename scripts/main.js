  ...
  window.RuntimeInterface = class RuntimeInterface {
    constructor(opts) {
      ...
      if (IsWebViewExportType(this._exportType) && this._useWorker) {
        console.warn(
          "[C3 runtime] Worker mode is enabled and supported, but is currently disabled in WebViews. Reverting to DOM mode."
        );
        this._useWorker = false;
      }
    ...
    async _MaybeGetCordovaScriptURL(url) {
      if (
        ...
        (url.startsWith("file:") ||
          (location.protocol === "file:" && this.IsRelativeURL(url)))
      ) {
        let filename = url;
